...
$CONTIG='$contig', Quality $CONTIG='$Qcontig'\n";
          exit 2;
          }
        }
      else
...
#printf STDOUT "min_bases=%d, max_bases=%d, divisor=%d, %d scale_counts=('%s')\n\n", $min_bases, $max_bases, $divisor, 
scalar keys %scale_counts, join("', '", map { "$_=$scale_counts{$_}" } (sort { $a <=> $b } keys %scale_counts));
...
#printf STDOUT "min_bases=%d, max_bases=%d, divisor=%d, %d bins=('%s')\n\n", $min_bases, $max_bases, $divisor, scalar keys 
%bins, join("', '", map { "$_=$bins{$_}" } (sort { $a <=> $b } keys %bins));
