...
#########################3           SNP PICKER VERSION SEBASTIEN        #########################################
##########################################################################################################
######
...
######		This will read in a VCF file (previously filtered to be in gene region and not a REPEAT), a reference genome, teh reference genome GFF file and total Num of NT in genome. 
	...
	#	USAGE:		 python /home/rreid2/scripts/python/snpPicker-genesOfInterest-seb.py $chr ./${chr}.vcf /nobackup/banana_genome/genesOfInterest/sebBananaIds.txt /nobackup/banana_genome/snppicker/chrfasta/${chr}.fna
...
while 1:
	...
	if pattern.match(line):
		...
		if "\'ID=Ma" in infocol:
			tmpray=infocol.split(";")[0].split(".")[0]
			ncbi=tmpray.replace("[\'ID=","")
			#ncbi=ncbi.replace(".1","")
			print ncbi
			#sys.exit()
