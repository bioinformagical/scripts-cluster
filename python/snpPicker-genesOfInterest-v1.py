...
#########################3           SNP PICKER VERSION GENE of INTEREST        #########################################
##########################################################################################################
######
...
######		This will read in a VCF file (previously filtered to be in gene region and not a REPEAT), a reference genome, teh reference genome GFF file and total Num of NT in genome. 
	...
	#	USAGE:		 python ~/scripts/python/snpPicker-genesOfInterest-v1.py $chr ./${chr}.vcf /nobackup/banana_genome/genesOfInterest/allansGeneListHits-jUSTNO.txt /nobackup/banana_genome/snppicker/chrfasta/${chr}.fna
...
######   OPEN THAT GOI file as a BIG Dict    #####
gffArray = []
goihash = {}
seq = ""
with open(sys.argv[3]) as goifile:
		...
		infoline=linearray[3]
		if "GeneID:" in infoline:
			foo=infoline.replace(';','GeneID:')
			foo=foo.replace(',','GeneID:')
			bar=foo.split("GeneID:")
			entrezid=bar[3]
			if tmp in location:
				coord=location.split("-")[1]
				goihash[entrezid] = coord
				activesequencename = line[1:]
				#print "We found our Entrez Id:",entrezid
				#print "We found our Cords:",coord
		if tmp in location:
			coord=location.split("-")[0]
			goihash[entrezid] = coord
			activesequencename = line[1:]
#sys.exit()
	...
	if pattern.match(line):
		...
		if "GeneID:" in infocol:
			...
			#ncbi = ncbi.replace("Genbank:","")
	        	if ncbi in goihash:
				print "We found a match for GOIHash:",ncbi," for the line ",tmpray
