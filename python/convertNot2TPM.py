...
#########################3    convertNot2TPM.py ----This will read in SRA 12 gene expression table from James       #########################################
##########################################################################################################
######
######		This will read in a 12 column gene expression table from James. And we will read in a TIAG GFF file.  
	...
	#		2.  Iterate through the gene expression file. If > 0, we will write the line
	#		3. Write results to table.
	#	USAGE:		 python /users/rreid2/scripts/python/convertNot2TPM.py /nobackup/banana_genome/tomato/publicData/cornell-other/ITAG3.2_gene_models.gff /nobackup/banana_genome/tomato/snps-12genesXpressed/12lyco-expressed-2.txt geneNagMalM82-snps.txt 
Prefix = ""
print sys.argv[1]
...
with open(sys.argv[3]) as snpfile:
	for line in snpfile:
		...
		if geneid in genesof12hash:
			outfile.write(genesof12hash[geneid])
			outfile.write("\n")
		else:
