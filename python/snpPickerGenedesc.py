...
#########################3           SNP PICKER VERSION GENE Description        #########################################
##########################################################################################################
######
######			Same as Version 2 but now we output a Gene description table friendly format.
######			For every VCF, check if we added the gene ID. Add to hash if needed. If new and we are adding it as SNP, write to dene desc file. 
######		This will read in a VCF file (previously filtered to be in gene region and not a REPEAT), a reference genome, teh reference genome GFF file and total Num of NT in genome. 
	...
	#	USAGE:		 python /home/rreid2/scripts/python/snpPicker-venki.py $chr ./${chr}.vcf /nobackup/banana_genome/snppicker/pahang-ref-just11chr.fna  331812599
...
front=tmparray[0]
### OUtfile
outname = front+"-genedesc.txt"
outfile = open(outname, 'w')
descname=front+"-descALL.txt"
descfile=open(descname, 'w')
counthash = {}

def getGeneDesc(snpline):
       larray=snpline.split(":")
       #print snpline
       desc=larray[0]
       return desc
...
while 1:
	...
	if pattern.match(line):
		...
		if (int(pos) > int(chromintervalarray[currentpos])):
			...
			descfile.write(locus)
			descfile.write("\tYES\t")
			descfile.write(chomrosome)
			#descfile.write("\t")
			#descfile.write((coord)
			#descfile.write("\t")
			descfile.write(infocol)
			descfile.write("\n")
		else:
			start=pos-49
			stop=pos+50
			snpseq=fasta[start:stop]
			if infocol in deschash:
				continue
			descfile.write(locus)
			descfile.write("\tNO\t")
			#descfile.write(chomrosome)
