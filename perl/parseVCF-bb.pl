...
my $file = $ARGV[0];
my $position = $ARGV[1] or die "We need a number for the file position";
my $file2 = $file.".vcf";
print $file2,"\n";
open(INPUTFILE, $file2) or die "can't read file";

my $outfile = $file."-table.txt";
open OUTFILE, ">$outfile" or die "No open outfile" ;

my $outfile2 = $file."-4bigtable.txt";
open OUTFILE2, ">$outfile2" or die "No open outfile" ;
...
# Set up format for output lines
		...
		######## Print for big table also    ########	
		print OUTFILE2 "$VCFfileArray[0]-$VCFfileArray[1]\t$file\t$position\n";
		}
