...
# This reads in a tabbed marker file and  makes a new fasta file..
#
#
...
while (my $line = <BLASTFILE>)
{
	...
	chomp $second;
	my $seq = $array[2];
	if ( exists $hash{$second})
	{
		#print "FOR $second , IN HASH: $hash{$second}"," and just found $seq\n";  
		if (! $hash{$second} eq $seq) {print "FOR $second , IN HASH: $hash{$second}"," and just found $seq\n"; }
	}
	else
	{
		$hash{$second}=$seq;
