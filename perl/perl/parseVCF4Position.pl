...
# 	Also making a seq-position id which we can to find all the unique VCF locations across progeny (A subsequent step not done here (can be done with awk then).
#	
#
...
# Set up format for output lines
		if ($dpvalue > 0 && $gq > 15)
		{
			my $Format = "$file\t$VCFfileArray[0]__$VCFfileArray[1]\t$VCFfileArray[3]\t$VCFfileArray[4]\t$VCFfileArray[5]\t$dpvalue\t$gt\t$genotype\t$pl\t$gq";
