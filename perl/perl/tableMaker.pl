	...
	#check if already exists in MASTERTABLE
	 if (exists $array33hash{$id})
	 {
		...
		for($i = 0; $i < 33; $i++)
		{
		 ...
		 $tempfo[$column] = $readnum; # column corresponding to file that came in, is updated.
		$array33hash{$id}[$column] = $readnum; # \@tempfo; # Update the master hash
	 ...
	 else
	 {
		...
		$temp[$column] = $readnum; # column corresponding to file that came in, is updated.
		#print "\n",$temp[34],"\n";
