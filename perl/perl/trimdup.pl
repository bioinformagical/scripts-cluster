...
foreach my $input_file ( @files ){
	...
	#my $output_file = $filename . ".trim" . $phrd_cutoff;
	my $output_file = $filename . ".dup";  
	...
	while( <INPUT> ){
		...
		my $adaptor02= "AGATCGGAAG";
		my $adaptor01= "GATCGGAAGAGCGGT";
		my $flag= "F";
		my $flag01= "F";
		my $dub_flag= "F";
		
		if(exists $dup{$seq_string})
		{
		$dub_flag= "F";
		
		}
		else{$dup{$seq_string} = "T";}
		
		if($seq_string=~ m/$adaptor02/)
		{
		$flag= "F";
		}
		
		if($seq_string=~ m/$adaptor01/)
		{
		$flag01= "F";
		}
        	 my $last20 = substr $seq_string, -20;
                if($last20=~ m/AGATC|GATCG|ATCGG|TCGGA|CGGAA|GGAAG|GAAGA/)
                {
                $flag01= "F";
		#print "Ahha, we found a possible adapter !", $last20,"\n";
                }
		...
		if( scalar( @seq_array ) == 0 or $flag01 eq  "T" or $flag eq  "T" or $dub_flag eq "T"){

			@quality_array="";
			@seq_array="";
			$seq_array[0]     = "N";
			$quality_array[0] = "B";
			print "OUr \@seq array has been reduced to N rubble",;
		}
