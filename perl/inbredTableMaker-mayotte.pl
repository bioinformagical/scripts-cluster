...
######    This script works only with Mayotte flies, not the D. sant. Seek other script for the D.sant files.
##		 perl ~/scripts/perl/inbredTableMaker-mayotte.pl 15_S33  ../mastertable/masterTemplate.orig
...
my @splitidray= split('_', $ARGV[0]);
my $splitid=$splitidray[0];


my $vcffile= "/nobackup/rogers_research/rob/mayotte/gatk/".$ARGV[0]."-gatk.vcf" or die "VCF FILE OPEN \n I need a VCF file..s. \n";
my $hapfile= "/nobackup/rogers_research/rob/mayotte/newmerge/".$splitid."_master.txt" or die "Feed me hap file ...";
my $masterfile= $ARGV[1] or die "Feed me master file so I can figure out if it exists as SNP zone ...";
